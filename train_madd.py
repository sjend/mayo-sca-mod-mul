...
def _create_model(input_size, learning_rate, epsilon):
    ...
    output = Dense(2**4, kernel_initializer='he_uniform', activation='softmax')(x)
...
def _load_dataset():
    raw_traces = np.load('madd/train_traces.npy')
    raw_labels = np.load('madd/train_labels.npy')
    ...
    labels = to_categorical(labels, num_classes=2**4)
...
tf.keras.models.save_model(model, 'madd/new')